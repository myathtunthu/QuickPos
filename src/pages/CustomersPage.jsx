  ...
  if (!hasPermission('view_customers')) {
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center text-slate-500">
        ...
        <p className="mt-2 text-sm">{t('noCustomerAccess')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 pb-20 text-white sm:p-6">
        ...
        <div className="flex w-full flex-col gap-3 md:w-auto sm:flex-row">
          ...
          {activeTab === 'book' && (
            <div className="flex gap-2">
              {isAdmin && (
                <>
                  <button type="button" onClick={handleExportCSV} className="rounded-xl bg-emerald-600/20 p-3 text-emerald-400 transition-colors hover:bg-emerald-600/40" title={t('exportCSV')}><Download size={20} /></button>
                  <button type="button" onClick={() => fileRef.current?.click()} className="rounded-xl bg-amber-600/20 p-3 text-amber-400 transition-colors hover:bg-amber-600/40" title={t('importCSV')}><Upload size={20} /></button>
        ...
        {activeTab === 'book' ? (
          <>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredCustomers.length === 0 ? (
                    <tr><td colSpan="5" className="p-8 text-center text-slate-500">{t('noCustomersYet')}</td></tr>
                      ...
                      <tr key={customer.id} className="transition-colors hover:bg-white/[0.02]">
                        ...
                        <td className="p-4 text-center">
                          <div className="flex justify-center gap-2">
                            <button type="button" onClick={() => { setSelectedCustomer(customer); setLedgerModalOpen(true); }} className="rounded-lg bg-blue-600/20 p-2 text-blue-400 transition-colors hover:bg-blue-600/40" title={t('ledger')}><ClipboardList size={16} /></button>
                            {canAcceptPayment && (
                              <button type="button" onClick={() => { setSelectedCustomer(customer); setPaymentForm(emptyPaymentForm); setPaymentModalOpen(true); }} disabled={debt <= 0} className={`rounded-lg p-2 transition-colors ${debt > 0 ? 'bg-amber-600/20 text-amber-400 hover:bg-amber-600/40' : 'cursor-not-allowed bg-gray-800 text-gray-600'}`} title={t('payment')}><DollarSign size={16} /></button>
                            )}
                            {canManageCustomers && (
                              <>
                                <button type="button" onClick={() => openEditCustomer(customer)} className="rounded-lg bg-indigo-600/20 p-2 text-indigo-400 transition-colors hover:bg-indigo-600/40" title={t('edit')}><Edit3 size={16} /></button>
                                <button type="button" onClick={() => handleDeleteCustomer(customer)} className="rounded-lg bg-rose-600/20 p-2 text-rose-400 transition-colors hover:bg-rose-600/40" title={t('delete')}><Trash2 size={16} /></button>
                              </>
                            )}
                          </div>
                        </td>
            ...
            <div className="block divide-y divide-white/5 sm:hidden">
              {filteredCustomers.length === 0 ? (
                <div className="p-8 text-center text-slate-500">{t('noCustomersYet')}</div>
              ) : visibleCustomers.map((customer) => {
                const debt = toMoney(customer.totalDebt);
                  ...
                  <div key={customer.id} className="p-4">
                    <button type="button" onClick={() => toggleCust(customer.id)} className="flex w-full items-start justify-between gap-3 text-left">
                      <div>
                        ...
                        <p className={`mt-2 text-sm font-black ${debt > 0 ? 'text-amber-400' : 'text-green-500'}`}>{debt > 0 ? formatMoney(debt) : t('cleared')}</p>
                      </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-white/5 bg-black/40 text-slate-400">
                <tr>
                  ...
                  <th className="p-4 text-center text-xs font-bold uppercase tracking-wider">{t('times')}</th>
      ...
      {activeTab === 'book' && filteredCustomers.length > visibleCustomers.length && (
        <div className="flex justify-center"><button type="button" onClick={() => setVisibleLimit((prev) => prev + CUSTOMER_RENDER_PAGE_SIZE)} className="rounded-xl border border-cyan-500/20 bg-cyan-600/20 px-5 py-3 font-bold text-cyan-300 hover:bg-cyan-600/30">{t('loadMore')} ({visibleCustomers.length}/{filteredCustomers.length})</button></div>
      )}

      {activeTab === 'history' && mergedHistory.length > visibleHistory.length && (
        <div className="flex justify-center"><button type="button" onClick={() => setHistoryVisibleLimit((prev) => prev + CUSTOMER_RENDER_PAGE_SIZE)} className="rounded-xl border border-purple-500/20 bg-purple-600/20 px-5 py-3 font-bold text-purple-300 hover:bg-purple-600/30">{t('loadMore')} ({visibleHistory.length}/{mergedHistory.length})</button></div>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <form onSubmit={handleSaveCustomer} className="w-full max-w-md rounded-3xl border border-cyan-500/30 bg-[#0d1120] p-6 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-xl font-black tracking-wide text-cyan-400">{editingCustomer ? t('editCustomer') : t('addCustomer')}</h3>
      ...
      {isPaymentModalOpen && selectedCustomer && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <form onSubmit={handlePayment} className="w-full max-w-sm rounded-3xl border border-amber-500/30 bg-[#0d1120] p-6 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-xl font-black tracking-wide text-amber-400">{t('paymentRecord')}</h3>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="flex max-h-[85vh] w-full max-w-2xl flex-col rounded-3xl border border-blue-500/30 bg-[#0d1120] shadow-2xl">
            <div className="flex items-center justify-between rounded-t-3xl border-b border-white/5 bg-black/20 p-6 pb-4">
              <div><h3 className="flex items-center gap-2 text-xl font-black text-blue-400"><ClipboardList size={20} /> {selectedCustomer.name}</h3><p className="mt-1 text-xs font-bold tracking-wider text-slate-400">{t('currentDebt')} <span className="text-sm text-amber-400">{formatMoney(selectedCustomer.totalDebt)}</span></p></div>
            ...
            <div className="custom-scrollbar flex-1 overflow-y-auto bg-black/10 p-4 sm:p-6">
              {currentLedger.length === 0 ? (
                <div className="py-10 text-center opacity-50"><p className="font-bold text-slate-400">{t('noRecords')}</p></div>
              ) : (
                  ...
                  {currentLedger.map((record) => {
                    ...
                    return (
                      <div key={record.id} onClick={() => isSale && setReceiptModal({ show: true, record })} className={`flex flex-col justify-between gap-3 rounded-2xl border border-white/5 bg-[#12182b] p-4 transition-colors sm:flex-row sm:items-center ${isSale ? 'cursor-pointer hover:border-cyan-500/50 hover:bg-[#1a2235]' : 'hover:border-blue-500/30'}`}>
                        <div>
                          <div className="mb-2 flex items-center gap-2"><span className={`rounded border px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${!isSale ? 'border-green-500/20 bg-green-500/20 text-green-400' : 'border-amber-500/20 bg-amber-500/20 text-amber-400'}`}>{!isSale ? t('paymentIn') : t('creditSale')}</span><span className="text-[11px] font-bold text-slate-500">{record.date} {record.time}</span></div>
                        ...
                        <div className="mt-2 border-t border-white/5 pt-2 text-left sm:mt-0 sm:border-0 sm:pt-0 sm:text-right">
                          <p className="text-[9px] font-bold uppercase tracking-wider text-slate-500">{!isSale ? t('amountReceived') : t('debtAdded')}</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 backdrop-blur-sm print:hidden">
          <div className="custom-scrollbar relative max-h-[90vh] w-full max-w-sm overflow-y-auto rounded-xl bg-white p-6 font-sans text-black shadow-2xl">
            ...
            <div className="mt-3 space-y-1 border-t border-gray-300 pt-3 text-xs">
              ...
              <div className="flex justify-between text-gray-600"><span>{t('paid')}</span><span>{formatMoney(receiptModal.record.paidAmount)}</span></div>
              <div className="mt-1.5 flex justify-between border-t border-gray-200 pt-1.5 font-bold text-red-600"><span>{t('credit')}</span><span>{formatMoney(receiptModal.record.remainingDebt)}</span></div>
            </div>
          </div>
        </div>
      )}
