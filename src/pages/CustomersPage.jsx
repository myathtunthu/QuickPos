...
export default function CustomersPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const recordData = recordsSnap.docs
        .map((snap) => ({ id: snap.id, ...snap.data() }))
        .sort((a, b) => {
      ...
      setPaymentRecords(recordData.filter((record) => record.type === 'Customer Payment'));
      setCreditSaleRecords(recordData.filter((record) => record.type === 'Sale' && toMoney(record.remainingDebt) > 0));
    } catch (error) {
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-4 pb-20 text-white sm:p-6">
      ...
      {duplicateCustomerWarnings.length > 0 && (
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-xs text-amber-200">
          {t('duplicateCustomerWarning', 'Duplicate customer records may exist. Auto-merge is disabled. Please review manually.')} {duplicateCustomerWarnings.length}
        </div>
      )}
                  ...
                  {filteredCustomers.length === 0 ? (
                    ...
                    return (
                      <tr key={customer.id} className="transition-colors hover:bg-white/[0.02]">
                        ...
                        <td className="p-4 text-right">{debt > 0 ? <span className={`text-base font-black ${isOverLimit ? 'text-rose-400' : 'text-amber-400'}`}>{formatMoney(debt)}</span> : <span className="text-sm font-bold text-green-500">{t('cleared')}</span>}</td>
                        <td className="p-4 text-center">
                          <div className="flex justify-center gap-2">
                            ...
                            {canAcceptPayment && (
                              <button type="button" onClick={() => { setSelectedCustomer(customer); setPaymentForm(emptyPaymentForm); setPaymentModalOpen(true); }} disabled={debt <= 0} className={`rounded-lg p-2 transition-colors ${debt > 0 ? 'bg-amber-600/20 text-amber-400 hover:bg-amber-600/40' : 'cursor-not-allowed bg-gray-800 text-gray-600'}`} title={t('paymentRecord')}><DollarSign size={16} /></button>
                            )}
                            {canManageCustomers && (
                              <>
                  ...
                  <div key={customer.id} className="p-4">
                    ...
                    {isExpanded && (
                      <div className="mt-4 space-y-3 rounded-2xl bg-black/30 p-3 text-sm">
                        <p className="text-slate-400">{t('address')}: {customer.address || '-'}</p>
                        <p className="text-slate-400">Credit Limit: {toMoney(customer.creditLimit) > 0 ? formatMoney(customer.creditLimit) : '-'}</p>
                        {customer.note && <p className="text-slate-400">{t('note')}: {customer.note}</p>}
                    ...
                    return (
                      <div key={record.id} onClick={() => isSale && setReceiptModal({ show: true, record })} className={`flex flex-col justify-between gap-3 rounded-2xl border border-white/5 bg-[#12182b] p-4 transition-colors sm:flex-row sm:items-center ${isSale ? 'cursor-pointer hover:border-cyan-500/50 hover:bg-[#1a2235]' : 'hover:border-blue-500/30'}`}>
                        <div>
                          ...
                          {isSale && <p className="mt-1 text-[11px] font-bold text-slate-500">{t('totalBill')} {formatMoney(record.amount)} • {t('paid')} {formatMoney(record.paidAmount)}</p>}
                        </div>
                        <div className="mt-2 border-t border-white/5 pt-2 text-left sm:mt-0 sm:border-0 sm:pt-0 sm:text-right">
                          ...
                          <p className="mt-1 text-[10px] text-slate-500">{t('balance')} {formatMoney(record.runningBalance)}</p>
                        </div>
                      </div>
                    );
                  })}
