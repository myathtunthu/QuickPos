...
export default function CustomersPage() {
  ...
  useEffect(() => {
    if (customers.length > 0 && allRecords.length > 0 && !autoMergeDone) checkAndMergeDuplicates();
  ...
  const mergedHistory = useMemo(() => {
    ...
    payments.forEach(p => {
      ...
      if (!merged[cId]) {
        merged[cId] = { customerId: p.customerId, personName: p.personName, totalPaid: 0, paymentCount: 0, lastPaymentDate: p.date, details: [] };
      }
  ...
  const handlePayment = async (e) => {
    ...
    if (payAmount > selectedCustomer.totalDebt + 10) return showToast("ဆပ်သည့်ငွေသည် အကြွေးထက် များနေပါသည်။", "error");
    ...
    try {
      ...
      await batch.commit(); setPaymentModalOpen(false); showToast("ငွေသွင်းမှတ်တမ်း သိမ်းပြီးပါပြီ", "success"); fetchData();
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          {activeTab === 'book' && (
            <div className="flex gap-2">
              ...
              <button onClick={() => { setEditingCustomer(null); setCustomerForm({ name: '', phone: '', address: '' }); setCustomerModalOpen(true); }} className="bg-cyan-600 text-white px-5 py-3 rounded-xl font-bold flex justify-center items-center gap-2 hover:bg-cyan-500 transition-colors shadow-lg active:scale-95">
                <Plus size={20}/> Add
              </button>
            </div>
          )}
        ...
        {activeTab === 'book' && (
          <>
            ...
            <div className="block sm:hidden divide-y divide-white/5">
              ...
              {!loading && filteredCustomers.length === 0 && <div className="p-8 text-center text-slate-500">Customer မတွေ့ပါ။</div>}
              {filteredCustomers.map(c => {
                    ...
                    <div onClick={() => toggleCust(c.id)} className="p-4 flex justify-between items-center cursor-pointer">
                      ...
                      <div className="text-right flex flex-col items-end">
                        ...
                        {Number(c.totalDebt) > 0 ? (
                           <p className="font-black text-amber-400 text-lg leading-none">{Number(c.totalDebt).toLocaleString()} Ks</p>
                        ) : (
          ...
          <>
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-black/40 text-slate-400 border-b border-white/5">
                  <tr>
                    <th className="p-4 font-bold uppercase tracking-wider text-xs">Customer Name</th>
            ...
            {/* 🌟 Mobile Accordion View */}
            <div className="block sm:hidden divide-y divide-white/5">
              {mergedHistory.length === 0 && <div className="p-8 text-center text-slate-500">ငွေသွင်းမှတ်တမ်း မရှိသေးပါ။</div>}
              {mergedHistory.map((h, i) => {
                ...
                return (
                  <div key={i} className="bg-[#0d1120] transition-colors">
                    ...
                    {isExpanded && (
                      <div className="p-4 bg-black/40 border-t border-cyan-500/10 flex justify-between items-center">
                        <span className="text-xs text-slate-400 font-bold">Total Payment Count:</span>
                        <span className="text-sm font-black text-cyan-400 bg-cyan-900/30 px-3 py-1 rounded-lg">{h.paymentCount} ကြိမ်</span>
                      </div>
                    )}
                  </div>
                )
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <form onSubmit={handleSaveCustomer} className="bg-[#0d1120] border border-cyan-500/30 rounded-3xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-cyan-400 tracking-wide">{editingCustomer ? 'Edit Customer' : 'Add Customer'}</h3>
              <button type="button" onClick={() => setCustomerModalOpen(false)} className="text-slate-400 hover:text-white p-1 bg-white/5 rounded-full"><X size={20}/></button>
            </div>
            <div className="space-y-4">
              ...
              <div><label className="text-xs text-slate-400 font-bold ml-1 mb-1 block">လိပ်စာ (Optional)</label><textarea value={customerForm.address} onChange={e=>setCustomerForm({...customerForm, address: e.target.value})} className="w-full bg-black/50 border border-cyan-500/20 rounded-xl p-3.5 text-white outline-none focus:border-cyan-400 text-sm custom-scrollbar" rows="2"></textarea></div>
            </div>
            <button type="submit" disabled={loading} className="w-full mt-8 bg-cyan-600 text-white font-black py-3.5 rounded-xl active:scale-95 transition-transform shadow-lg shadow-cyan-900/50">သိမ်းမည်</button>
          </form>
        </div>
      )}
      ...
      {isPaymentModalOpen && selectedCustomer && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <form onSubmit={handlePayment} className="bg-[#0d1120] border border-amber-500/30 rounded-3xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-amber-400 tracking-wide">ငွေသွင်းမှတ်တမ်း</h3>
            ...
            <div className="bg-black/40 p-5 rounded-2xl mb-6 text-center border border-white/5 shadow-inner">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Credit Balance</p>
            ...
            <button type="submit" disabled={loading} className="w-full mt-8 bg-amber-600 text-white font-black py-4 rounded-xl active:scale-95 transition-transform shadow-lg shadow-amber-900/50">ငွေသွင်းမည်</button>
          </form>
        </div>
      )}
          ...
          <div className="bg-[#0d1120] border border-blue-500/30 rounded-3xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl">
            <div className="p-6 pb-4 border-b border-white/5 flex justify-between items-center bg-black/20 rounded-t-3xl">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 font-bold tracking-wider">Current Debt: <span className="text-amber-400 text-sm">{Number(selectedCustomer.totalDebt).toLocaleString()} Ks</span></p>
              </div>
                      ...
                      <div key={record.id} onClick={() => isSale && setReceiptModal({ show: true, record })} className={`bg-[#12182b] border border-white/5 p-4 rounded-2xl flex flex-col sm:flex-row justify-between sm:items-center gap-3 transition-colors ${isSale ? 'cursor-pointer hover:border-cyan-500/50 hover:bg-[#1a2235]' : 'hover:border-blue-500/30'}`}>
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider ${!isSale ? 'bg-green-500/20 text-green-400 border border-green-500/20' : 'bg-amber-500/20 text-amber-400 border border-amber-500/20'}`}>{!isSale ? 'Payment In' : 'Credit Sale'}</span>
                        ...
                        <div className="text-left sm:text-right pt-2 sm:pt-0 border-t border-white/5 sm:border-0 mt-2 sm:mt-0">
                          ...
                          {!isSale ? (<p className="text-lg font-black text-green-400 mt-0.5">-{Number(record.amount).toLocaleString()} <span className="text-xs">Ks</span></p>) : (<p className="text-lg font-black text-amber-400 mt-0.5">+{Number(record.remainingDebt).toLocaleString()} <span className="text-xs">Ks</span></p>)}
        ...
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm print:hidden">
          <div className="w-full max-w-sm bg-white text-black rounded-xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar font-sans relative animate-in zoom-in-95">
            ...
            <div className="border-t border-gray-300 pt-3 mt-3 space-y-1 text-xs">
               ...
               <div className="flex justify-between text-red-600 font-bold border-t border-gray-200 pt-1.5 mt-1.5"><span>Credit (အကြွေးကျန်):</span><span>{Number(receiptModal.record.remainingDebt).toLocaleString()} Ks</span></div>
            </div>
          </div>
        </div>
      )}
