...
export default function EntryPage({ products = [] }) {
  ...
  // ✅ NEW: Price Type & Packing Unit
  const [priceType, setPriceType] = useState('retail'); // retail / wholesale
  const [packingUnit, setPackingUnit] = useState('pcs'); // pcs, pack, bundle, dozen
  ...
  // ✅ Helper: Get Product Price based on Price Type + Packing
  const getProductPrice = (product) => {
    if (!product) return 0;
    let basePrice = priceType === 'wholesale' ? (product.wholesalePrice || product.price || 0) : (product.price || 0);
    switch (packingUnit) {
      case 'pack': return basePrice * (product.packQty || 6);
      case 'bundle': return basePrice * (product.bundleQty || 12);
      case 'dozen': return basePrice * 12;
      default: return basePrice;
    }
  };

  const getPackingLabel = () => {
    switch (packingUnit) {
  ...
  const doPrint = (record) => {
    ...
    ${items.map(i => `<tr><td>${i.name}${i.packingUnit&&i.packingUnit!=='pcs'?`<br><small>(${i.packingUnit})</small>`:''}${i.priceType==='wholesale'?'<br><small>လက်ကား</small>':''}${i.itemDiscountAmt>0?'<br><small>Disc: -'+fmt(i.itemDiscountAmt)+'</small>':''}</td><td>${i.quantity}</td><td>${fmt(i.unitPrice)}</td><td>${fmt(i.unitPrice*i.quantity-(i.itemDiscountAmt||0))}</td></tr>`).join('')}
    </tbody></table>
  ...
  return (
    <div className="p-4 pb-28 text-white max-w-5xl mx-auto space-y-5 bg-[#080c14] min-h-screen">
      ...
      {/* ✅ Price Type & Packing Unit Selector */}
      <div className="flex gap-2 flex-wrap">
        <div className="flex bg-black/40 border border-cyan-500/20 rounded-xl overflow-hidden">
          <button onClick={() => setPriceType('retail')} className={`px-4 py-2 text-sm font-bold ${priceType==='retail'?'bg-cyan-600 text-white':'text-slate-400'}`}>လက်လီ</button>
          <button onClick={() => setPriceType('wholesale')} className={`px-4 py-2 text-sm font-bold ${priceType==='wholesale'?'bg-cyan-600 text-white':'text-slate-400'}`}>လက်ကား</button>
        </div>
        <div className="flex bg-black/40 border border-cyan-500/20 rounded-xl overflow-hidden">
          {[{key:'pcs',label:'ခု'},{key:'pack',label:'ဗူး'},{key:'bundle',label:'တွဲ'},{key:'dozen',label:'ဒါဇင်'}].map(u => (
            <button key={u.key} onClick={() => setPackingUnit(u.key)} className={`px-3 py-2 text-xs font-bold ${packingUnit===u.key?'bg-emerald-600 text-white':'text-slate-400'}`}>{u.label}</button>
          ))}
        </div>
      </div>
        ...
        <>
          ...
          {cart.length > 0 && (
            <div className="space-y-3">
              {cart.map(item => (<div key={item.id} className="bg-black/40 border border-cyan-500/10 rounded-xl p-4"><div className="flex justify-between"><div><p className="font-black">{item.name}</p><p className="text-cyan-400 text-sm">{fmt(item.unitPrice)} × {item.quantity} = {fmt(item.unitPrice*item.quantity)} Ks {item.packingUnit&&item.packingUnit!=='pcs'&&<span className="text-xs text-slate-500">({item.packingUnit})</span>}{item.priceType==='wholesale'&&<span className="text-xs text-amber-400">လက်ကား</span>}</p></div><button onClick={()=>removeFromCart(item.id)} className="text-rose-400"><Trash2 size={18}/></button></div><div className="mt-3 flex items-center gap-2 text-amber-400 text-sm"><Tag size={14}/> Disc:<input value={item.itemDiscountAmt||''} onChange={e=>updateItemDiscount(item.id,e.target.value)} placeholder="0" className="w-24 bg-black border border-amber-500/20 rounded-lg px-3 py-1.5 text-white text-sm" /> Ks</div></div>))}
