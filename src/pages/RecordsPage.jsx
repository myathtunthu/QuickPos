...
export default function RecordsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 text-white max-w-6xl mx-auto space-y-6 pb-10">
      <div className="bg-[#0d1120] p-5 sm:p-6 rounded-3xl border border-cyan-500/15 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-2xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
            <FileText size={26} />
          </div>
          <div>
            <p className="text-xs font-black uppercase tracking-[0.25em] text-cyan-400">{t('records_subtitle', 'Transaction history')}</p>
            <h1 className="text-3xl font-black text-white">{t('records_title', 'Records')}</h1>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col bg-[#0d1120] p-4 sm:p-6 rounded-3xl border border-cyan-500/15 shadow-xl gap-4">
        <div className="flex justify-between items-center">
          <h3 className="font-black text-xl sm:text-2xl flex items-center gap-2">
            <FileText className="text-cyan-500" size={24} /> {t('recentTransactions', 'Transactions')}
          </h3>
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text" placeholder={t('searchTransactions', 'Search by name, voucher...')}
          ...
          <div className="flex gap-2 overflow-x-auto">
            {[{ id: 'All', label: t('allRecords', 'All') }, { id: 'Sale', label: t('sale', 'Sale') }, { id: 'Purchase', label: t('purchase', 'Purchase') }, { id: 'Expense', label: t('expense', 'Expense') }].map(type => (
              <button
                key={type.id} onClick={() => setFilterType(type.id)}
                className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${
                  filterType === type.id ? 'bg-cyan-600 text-white' : 'bg-black/50 text-slate-400 border border-white/5'
                }`}
              >{type.label}</button>
            ))}
          </div>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-[#0d1120] rounded-2xl border border-dashed border-slate-500/20">
            ...
            <p className="text-slate-400 font-bold">{t('noTransactions', 'No transactions found in selected date range.')}</p>
          </div>
        ) : (
          filteredRecords.map(r => (
            ...
            >
              <div>
                ...
                <p className="font-bold text-white">{r.personName || r.item || t('walkInCustomer', 'Walk-in')}</p>
